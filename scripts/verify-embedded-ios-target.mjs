...
const root = resolve(
  process.cwd(),
  argumentValue("--root") ?? "ios/App/App/public",
);
...
try {
  ...
  if (forbiddenMatches.size > 0) {
    throw new Error(
      `iOS embedded assets contain forbidden non-Production refs: ${[
        ...forbiddenMatches,
      ].join(", ")}`,
    );
  }
  if (productionMatches === 0) {
    throw new Error(
      `iOS embedded assets do not contain Production ref ${PRODUCTION_REF}`,
    );
  }

  console.log(
    `iOS embedded target validation passed: Production ${PRODUCTION_REF}`,
  );
} catch (error) {
  console.error(
    error instanceof Error ? error.message : "iOS embedded target validation failed",
  );
  process.exit(1);
}

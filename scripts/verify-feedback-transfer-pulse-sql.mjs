...
const transferPulseCountMigration = readFileSync(
  resolve("supabase/migrations/20260810122749_feedback_intelligence_transfer_pulse_count_v0_2_1.sql"),
  "utf8",
);
...
const machineExportSchemaV021 = JSON.parse(readFileSync(
  resolve("docs/feedback-intelligence/contracts/v0.2.1/proposed-export.schema.json"),
  "utf8",
));
...
try {
  ...
  const upgradedDefinitions = await db.query(`
    ...
    WHERE procedure.oid IN (
      'public.read_feedback_intelligence_v0_2_draft(text,text,text,text)'::regprocedure,
      'feedback_analysis.export_feedback_intelligence_v0_2_internal(text,text,text,text)'::regprocedure,
      'feedback_core.capture_transfer_pulse_count_on_submit()'::regprocedure
    )
    ORDER BY namespace.nspname, procedure.proname
  `);
  const upgradedDefinitionHashes = Object.fromEntries(upgradedDefinitions.rows.map((row) => [
    `${row.schema_name}.${row.function_name}`,
    createHash("sha256").update(row.definition, "utf8").digest("hex"),
  ]));
  assert(
  ...
  const upgradedTransferSnapshot = await db.query(`
    SELECT snapshot.transfer_pulse_count, snapshot.source_contract_version
    FROM feedback_core.activity_snapshots snapshot
    INNER JOIN feedback_core.submissions submission ON submission.id = snapshot.submission_id
  ...
  await expectFailure(
    () => db.query(`
      ...
      SET transfer_pulse_count = 3
      FROM feedback_core.submissions submission
      WHERE submission.id = snapshot.submission_id
        AND submission.user_id = $1
        AND snapshot.observation_end_program_day = 10
    `, [machineUsers[0].user]),
    "activity_snapshots_transfer_pulse_count_check",
  );
  ...
  for (const deniedRole of ["anon", "authenticated", "service_role"]) {
    ...
    await expectFailure(
      () => db.query(`
        SELECT public.read_feedback_intelligence_v0_2_draft(
          'mahles-jarvis-feedback-intelligence', '0.2.1-draft',
  ...
  const gatewayCall = async (sequence, overrides = {}) => {
    ...
    `, [
      overrides.schema ?? "e90eb3fc2ce717ef91ae35bcfcd5bc7944d3cc941faa8f071b42e934e967023d",
  ...
  assert(
    new Ajv2020({ strict: false, validateFormats: false })
      .compile(machineExportSchemaV021)(gatewaySuccess.rows[0].result),
    "gateway success must match the v0.2.1 count-only transfer export schema",
  );
  assert(
    gatewaySuccess.rows[0].result.items.some((item) => item.activity_snapshot.transfer_pulse_count === 2)
      && gatewaySuccess.rows[0].result.items.every((item) =>
        Number.isInteger(item.activity_snapshot.transfer_pulse_count)
        && !("score" in item.activity_snapshot)
        && !("text" in item.activity_snapshot)),
    "gateway may export only bounded transfer counts, never transfer scores or text",
  );
  ...
  await db.exec(`
    UPDATE feedback_core.activity_snapshots
    SET transfer_pulse_count = 0,
        source_contract_version = 'feedback-activity-counts-v1.1.0'
    WHERE transfer_pulse_count IS NULL
  `);
  await db.exec(`
  ...
  assert(
    new Ajv2020({ strict: false, validateFormats: false })
      .compile(machineExportSchemaV021)(longitudinalPayload),
    "the full longitudinal fixture must match the byte-pinned v0.2.1 export schema",
  );
  for (const [checkpoint, maximum] of [[10, 2], [24, 6], [39, 11], [55, 15]]) {
    const validAtMaximum = structuredClone(longitudinalPayload);
    const validItem = validAtMaximum.items.find((item) => item.program_day === checkpoint);
    validItem.activity_snapshot.transfer_pulse_count = maximum;
    const validateAtMaximum = new Ajv2020({ strict: false, validateFormats: false })
      .compile(machineExportSchemaV021);
    assert(
      validateAtMaximum(validAtMaximum),
      `checkpoint day ${checkpoint} must accept transfer count ${maximum}`,
    );

    const invalidAboveMaximum = structuredClone(validAtMaximum);
    const invalidItem = invalidAboveMaximum.items.find((item) => item.program_day === checkpoint);
    invalidItem.activity_snapshot.transfer_pulse_count = maximum + 1;
    const validateAboveMaximum = new Ajv2020({ strict: false, validateFormats: false })
      .compile(machineExportSchemaV021);
    assert(
      validateAboveMaximum(invalidAboveMaximum) === false,
      `checkpoint day ${checkpoint} must reject transfer count ${maximum + 1}`,
    );
  }
  assert(

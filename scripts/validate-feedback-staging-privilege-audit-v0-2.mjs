...
const expectedDataPaths = {
  gateway: {
    ...
    function_settings: expectedSettings,
    return_type: "jsonb",
    volatility: "VOLATILE",
...
const matchesExactObject = (actual, expected) => Object.entries(expected)
  .every(([key, value]) => Array.isArray(value)
    ? JSON.stringify(actual[key]) === JSON.stringify(value)
    : actual[key] === value);
const matchesDataPath = (actual, expected) => actual?.present === true
  && matchesExactObject(actual, expected);
...
if (result.audit_phase === "PREDEPLOY_BASELINE") {
  ...
  if (gateway.present && !matchesDataPath(gateway, expectedDataPaths.gateway)) {
    add("PREDEPLOY_GATEWAY_FUNCTION_DRIFT", "NO_GO", "Existing predeploy gateway definition or security metadata differs from the exact pin.");
  }
  const internal = evidence.internal_export_function;
  if (internal.present && !matchesDataPath(internal, expectedDataPaths.internal)) {
    add("PREDEPLOY_INTERNAL_EXPORT_FUNCTION_DRIFT", "NO_GO", "Existing predeploy internal export definition or security metadata differs from the exact pin.");
  }
  ...
  const gateway = evidence.gateway_function;
  if (!matchesDataPath(gateway, expectedDataPaths.gateway)) {
    add("GATEWAY_FUNCTION_DRIFT", "NO_GO", "Gateway definition, signature, owner, definer, settings, return type, or volatility differs.");
  }
  const internal = evidence.internal_export_function;
  if (!matchesDataPath(internal, expectedDataPaths.internal)) {
    add("INTERNAL_EXPORT_FUNCTION_DRIFT", "NO_GO", "Internal export definition, signature, owner, definer, settings, return type, or volatility differs.");
  }
  const pinnedInventory = evidence.machine_export_functions.filter((fn) =>
    (fn.schema_name === "public" && fn.function_name === "read_feedback_intelligence_v0_2_draft")
    || (fn.schema_name === "feedback_analysis" && fn.function_name === "export_feedback_intelligence_v0_2_internal")
  );
  const inventoryMatches = pinnedInventory.length === 2
    && pinnedInventory.some((fn) => matchesExactObject(fn, {
      schema_name: "public",
      function_name: "read_feedback_intelligence_v0_2_draft",
      identity_arguments: "text, text, text, text",
      owner_name: "postgres",
      security_definer: true,
      function_settings: expectedSettings,
      return_type: "jsonb",
      volatility: "VOLATILE",
      definition_sha256: expectedDataPaths.gateway.definition_sha256,
    }))
    && pinnedInventory.some((fn) => matchesExactObject(fn, {
      schema_name: "feedback_analysis",
      function_name: "export_feedback_intelligence_v0_2_internal",
      identity_arguments: "text, text, text, text",
      owner_name: "postgres",
      security_definer: true,
      function_settings: expectedSettings,
      return_type: "jsonb",
      volatility: "VOLATILE",
      definition_sha256: expectedDataPaths.internal.definition_sha256,
    }));
  if (!inventoryMatches) {
    add("DATA_PATH_FUNCTION_INVENTORY_DRIFT", "NO_GO", "The audited Machine/Export inventory does not contain exactly both pinned data-path definitions.");
  }
  const matrix = evidence.gateway_execute_matrix;

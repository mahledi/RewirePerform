...
describe("V1.4 Block 9 privacy and activation contract", () => {
  it("pins the existing consent as a candidate, never as pre-approved compatibility", () => {
    ...
    expect(migration).toContain("'data_contribution_v3_2026_07'");
    expect(migration).toContain("'conditionally_compatible'");
    expect(migration).toContain("evidence_v1_4_block_9_incomplete");
    expect(migration).not.toMatch(/UPDATE\s+evidence_derived\.analysis_protocols\s+SET\s+status\s*=\s*'active'/i);
  });
  ...
  it("retains the stronger guardian wording while flagging the adult coach-observation gap", () => {
    const block9 = read("docs/evidence-v1.4/BLOCK_9_PRIVACY_COMPATIBILITY.md");
    ...
    expect(block9).toContain("separate Kompatibilitätsentscheidung oder neue Einwilligung");
  });

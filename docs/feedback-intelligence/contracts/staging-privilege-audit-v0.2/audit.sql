-- RewirePerform Feedback Intelligence metadata-only Staging privilege audit v0.2.
--
  ...
  SELECT jsonb_build_object(
    ...
    'gateway_function', COALESCE((
      SELECT jsonb_build_object(
        ...
        'return_type', gateway.return_type,
        'volatility', gateway.volatility,
        'definition_sha256', gateway.definition_sha256
      ) FROM gateway_function gateway
    ), jsonb_build_object('present', false)),
    'internal_export_function', COALESCE((
      SELECT jsonb_build_object(
        'present', true,
        'signature', internal.schema_name || '.' || internal.function_name || '(' || internal.identity_arguments || ')',
        'security_definer', internal.security_definer,
        'function_settings', to_jsonb(internal.function_settings),
        'owner_name', internal.owner_name,
        'owner_superuser', internal.owner_superuser,
        'owner_bypass_rls', internal.owner_bypass_rls,
        'return_type', internal.return_type,
        'volatility', internal.volatility,
        'definition_sha256', internal.definition_sha256
      ) FROM internal_export_function internal
    ), jsonb_build_object('present', false)),
...
SELECT jsonb_build_object(
  'schema_version', 'rewireperform-feedback-intelligence-staging-privilege-audit-result-v2',
  ...
  'audit_phase', CASE
    WHEN EXISTS (SELECT 1 FROM reader_role)
      AND EXISTS (SELECT 1 FROM gateway_function)
      AND EXISTS (SELECT 1 FROM internal_export_function)

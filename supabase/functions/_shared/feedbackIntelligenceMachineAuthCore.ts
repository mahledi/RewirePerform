...
const MACHINE_KEY_PATTERN = /^[a-f0-9]{64}$/u;

export type FeedbackIntelligenceMachineAuthError =
...
export const authenticateFeedbackIntelligenceAuthorization = async ({
  authorization,
  currentKey,
  previousKey,
}: {
  ...
  const match = /^Bearer ([a-f0-9]{64})$/u.exec(authorization.trim());

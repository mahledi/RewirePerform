...
BEGIN
  ...
  team_athletes AS (
    ...
    JOIN eligible_users eu ON eu.id = tm.user_id
  ),
  latest_snapshots AS (
    ...
    JOIN eligible_users eu ON eu.id = pps.user_id
  ...
  global_activity AS (
    SELECT
      ...
      (SELECT COUNT(*) FROM public.user_roles ur JOIN eligible_users eu ON eu.id = ur.user_id WHERE ur.role = 'athlete')::int AS athletes_total,
      ...
      (SELECT COUNT(*) FROM public.program_instances pi JOIN eligible_users eu ON eu.id = pi.user_id)::int AS program_instances_total,
      (SELECT COUNT(*) FROM public.user_day_completion udc JOIN eligible_users eu ON eu.id = udc.user_id WHERE udc.completion_status = 'completed')::int AS completed_days_total,
      (SELECT COUNT(*) FROM public.daily_checkins dc JOIN eligible_users eu ON eu.id = dc.user_id)::int AS checkins_total,
      (SELECT COUNT(*) FROM public.comprehension_check_instances cci JOIN eligible_users eu ON eu.id = cci.user_id WHERE cci.status = 'completed')::int AS comprehension_total,
      (SELECT COUNT(*) FROM public.daily_journals dj JOIN eligible_users eu ON eu.id = dj.user_id)::int AS journals_total,
      (SELECT COUNT(DISTINCT user_id) FROM (
        SELECT udc.user_id FROM public.user_day_completion udc JOIN eligible_users eu ON eu.id = udc.user_id WHERE udc.completed_at >= now() - interval '7 days'
        UNION
        SELECT dc.user_id FROM public.daily_checkins dc JOIN eligible_users eu ON eu.id = dc.user_id WHERE dc.created_at >= now() - interval '7 days'

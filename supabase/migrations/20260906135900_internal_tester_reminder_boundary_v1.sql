-- Internal tester boundary: coach check-in status and reminders.
BEGIN;
...
BEGIN
  ...
  WITH athletes AS (
    ...
    SELECT athlete.user_id, instance.id AS program_instance_id
    ...
    SELECT
      instance.user_id,
      MAX(checkin.created_at) FILTER (WHERE checkin.date = effective_today) AS today_checkin_at,
  ...
  ORDER BY (checkins.today_checkin_at IS NULL) DESC, profile.full_name NULLS LAST;
END;
$$;
...
BEGIN
  ...
  INSERT INTO app_private.coach_checkin_reminder_campaigns (
    team_id, program_run_id, requested_by, program_date, timezone, is_test
  ) VALUES (
    _team_id, active_run_id, _requested_by, effective_today, run_timezone, team_is_test
  )
  ...
  RETURN QUERY
  WITH eligible AS (
    SELECT DISTINCT member.user_id, instance.id AS program_instance_id
    FROM public.team_members member
    JOIN public.user_roles role
      ON role.user_id = member.user_id
     AND role.role = 'athlete'::public.app_role
    ...
    INSERT INTO public.notification_log (
      user_id, notification_type, sent_date, status,
      scheduled_for, target_url, metadata
    )
    SELECT
      eligible.user_id,

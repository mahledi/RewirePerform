-- V1.3 structured-only Jarvis producer with server-derived participation mode.
--
-- This is a new versioned contract. V1.2 stays immutable, but the dedicated
-- reader loses EXECUTE on V1.2 before it receives exactly one V1.3 RPC.
-- `participation_mode` is derived from program_instances inside Postgres. No
-- team, run, user or program-instance identifier crosses the reader boundary.
-- n >= 5 is enforced independently per question/version/mode group.
-- Applying this migration does not create credentials, open runtime gates or
-- read application data.

BEGIN;

CREATE OR REPLACE FUNCTION feedback_machine_production.read_feedback_intelligence_production_structured_v1_3(
  _client_id text,
...
BEGIN
  ...
  IF _client_id IS DISTINCT FROM 'mahles-jarvis-feedback-intelligence-production'
     OR _contract_version IS DISTINCT FROM '1.3.0-participation-mode-draft'
     OR _schema_sha256 IS DISTINCT FROM 'e666b8c48f5de2ab32154d7b4b347e9d3eefeaa49fed22f448a5f0e98202b516'
  ...
  WITH eligible_subjects AS MATERIALIZED (
    SELECT DISTINCT
      ...
      CASE
        WHEN instance.team_id IS NULL AND instance.program_run_id IS NULL THEN 'solo'
        ELSE 'team'
    ...
    WHERE submission.status = 'submitted'
      ...
      AND public.evidence_eligibility_reason(instance.id, '56d-transfer-v2-2026-07')
        IN ('eligible', 'eligible_minor')
  ), source_items AS MATERIALIZED (
    SELECT item.value, item.ordinality, eligible.participation_mode
    ...
    INNER JOIN allowed_groups allowed
      ON allowed.participation_mode = source.participation_mode
      AND allowed.questionnaire_version = source.value ->> 'questionnaire_version'
  ...
  SELECT COALESCE(pg_catalog.jsonb_agg(item ORDER BY ordinality), '[]'::jsonb)
  ...
  IF EXISTS (
    ...
    WHERE item ?| ARRAY[
      ...
      'user_id', 'program_instance_id', 'program_run_id', 'team_id', 'coach_id',
      'name', 'email', 'journal_text', 'reflection_text', 'support_text'
    ]
    OR item ->> 'participation_mode' NOT IN ('solo', 'team')
  ) THEN
...
REVOKE ALL ON FUNCTION feedback_machine_production.read_feedback_intelligence_production_structured_v1_3(
  text, text, text, text
) FROM PUBLIC, anon, authenticated, service_role, mahleos_feedback_reader;
GRANT EXECUTE ON FUNCTION feedback_machine_production.read_feedback_intelligence_production_structured_v1_3(
  text, text, text, text
...
COMMENT ON FUNCTION feedback_machine_production.read_feedback_intelligence_production_structured_v1_3(
  text, text, text, text
) IS
  'V1.3 structured-only Jarvis producer. Derives solo/team server-side without exporting team identifiers and enforces n>=5 per question/version/mode.';

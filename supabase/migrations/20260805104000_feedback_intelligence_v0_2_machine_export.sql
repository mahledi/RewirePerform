-- Feedback Intelligence v0.2 draft machine export.
--
-- This defines the exact read shape but grants execute to nobody. Every real
-- gate is false, there is no machine credential, transport, URL or scheduler,
-- and Production cannot be read. A later reviewed migration must provision a
-- dedicated role and grant this single function explicitly.

BEGIN;

CREATE TABLE feedback_core.machine_contract_settings (
  contract_version text PRIMARY KEY
    CHECK (contract_version = '0.2.0-draft'),
  schema_version text NOT NULL
    CHECK (schema_version = 'rewire-feedback-intelligence-export-v0.2-draft'),
  contract_status text NOT NULL
    CHECK (contract_status = 'PRODUCER_CONFIRMED_DRAFT_NOT_ACTIVATED'),
  schema_sha256 text NOT NULL CHECK (schema_sha256 ~ '^[a-f0-9]{64}$'),
  consumer_pin_ready boolean NOT NULL DEFAULT false,
  synthetic_export_enabled boolean NOT NULL DEFAULT false,
  production_export_enabled boolean NOT NULL DEFAULT false,
  machine_credential_ready boolean NOT NULL DEFAULT false,
  ...
  producer_package_sha256 text
    CHECK (producer_package_sha256 IS NULL OR producer_package_sha256 ~ '^[a-f0-9]{64}$'),
  updated_at timestamptz NOT NULL DEFAULT now()
);

INSERT INTO feedback_core.machine_contract_settings(
  contract_version,
  schema_version,
  contract_status,
  schema_sha256
  ...
  '0.2.0-draft',
  'rewire-feedback-intelligence-export-v0.2-draft',
  'PRODUCER_CONFIRMED_DRAFT_NOT_ACTIVATED',
  'fb1ef751bc4701a497f224bb421220e08b3387eba5c2eaec9e91e2cbf474b4e9'
)
ON CONFLICT (contract_version) DO NOTHING;

ALTER TABLE feedback_core.machine_contract_settings ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE feedback_core.machine_contract_settings
  FROM PUBLIC, anon, authenticated, service_role;

CREATE TRIGGER feedback_machine_contract_settings_touch_updated_at
BEFORE UPDATE ON feedback_core.machine_contract_settings
FOR EACH ROW EXECUTE FUNCTION feedback_core.touch_updated_at();

CREATE OR REPLACE FUNCTION feedback_core.export_reference_hash(
  _kind text,
  _value text
)
RETURNS text
...
AS $$
  SELECT encode(
    extensions.digest(
      convert_to(
        'rewire-feedback-intelligence-export-v0.2-draft:' || _kind || ':' || _value,
        'UTF8'
      ),
      'sha256'
    ),
...
CREATE OR REPLACE FUNCTION feedback_core.export_safe_product_version(_value text)
RETURNS text
LANGUAGE sql
IMMUTABLE
SECURITY DEFINER
    ...
    SELECT jsonb_build_object(
      ...
      'consent', jsonb_build_object(
        ...
        'consent_reference', CASE
          WHEN row.consent_reference IS NOT NULL
...
REVOKE ALL ON FUNCTION feedback_core.export_reference_hash(text, text)
  FROM PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION feedback_core.export_safe_product_version(text)
  FROM PUBLIC, anon, authenticated, service_role;
...
COMMENT ON FUNCTION public.read_feedback_intelligence_v0_2_draft(text, text, text, text) IS
  'Pinned v0.2 draft export. No role currently has execute; a separate reviewed credential/grant migration is required.';
COMMENT ON FUNCTION feedback_core.export_safe_product_version(text) IS
  'Explicit consumer-safe encoding: SemVer +build is exported as _build_; native stored value remains unchanged.';
